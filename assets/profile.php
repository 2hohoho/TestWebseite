...
$user_statement->bindParam("Benutzername", $loggedin_username);
...
if(isset($_POST['email'])) {
  ...
  $user_statement->bindParam("username", $loggedin_username);
  $user_statement->execute();
...
if(isset($_POST['password'], $_POST['password1'])) {
  ...
  if($password == $password1) {
    ...
    $user_statement->bindParam("username", $loggedin_username);
    $user_statement->execute();
    header("Location: /?message=logoutChange");
  } else {
    echo('
      ...
      <div class="card-body">
        <form method="post">
          <div class="form-group">
            <label>Rang:</label>
            <label class="form-control"><?php if(in_array($loggedin_username, $adminlist)) { echo("Administrator"); } else { echo("User"); } ?></label>
          </div>
        </form>
      </div>
    </div>
